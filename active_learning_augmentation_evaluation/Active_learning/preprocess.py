...
def detector_preprocess(
    ...
    if is_crop:
        img, bboxes, labels = _crop_with_bbox(img, _my_uniform(4 / 5, 5 / 6), bboxes, labels)
        pass
    if is_flip:
...
def _crop_with_bbox(img, crop_prop, bboxes, labels):
    ...
    """
    # Generate clipped coordinates
    width = img.size[0]
    height = img.size[1]
    crop_width = int(crop_prop * width)
    crop_height = int(crop_prop * height)
    crop_x_min = int(random.uniform(0, width - crop_width))
    crop_y_min = int(random.uniform(0, height - crop_height))
    crop_x_max = crop_x_min + crop_width
    crop_y_max = crop_y_min + crop_height
