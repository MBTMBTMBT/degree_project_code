...
def _crop_with_bbox(img, crop_prop, bboxes, labels):
    ...
    for idx, bbox in enumerate(bboxes):
        ...
        draw_0 = cv2.rectangle(img, (bbox[0],bbox[1]), (bbox[2],bbox[3]), (255, 0, 0), 2)
...
def _change_contrast(img):
    """
    The contrast will change randomly within a certain range 50%~150% (1-0.5~1+0.5)
    """
...
def _change_saturation(img):
    """
    The saturation will change randomly within a certain range 50%~150% (1-0.5~1+0.5)
    """
...
def _change_hue(img):
    """
    The hue will change randomly within a certain range 50%~150% (1-0.5~1+0.5)
    """
...
def _gaussian_noise(img, mean, sigma):
    # Image->cv2(np.adarray)
    img = np.array(img)

    # Standardize the image gray scale
    img = img / 255
    ...
    # Set everything above 1 to 1 and everything below 0 to 0
    gaussian_img = np.clip(gaussian_img, 0, 1)

    # Restore the grayscale range of the image to 0-255
    gaussian_img = np.uint8(gaussian_img * 255)

    # cv2(np.adarray)->Image对象
...
if __name__ == '__main__':
    ...
    # img_pre = detector_preprocess(img, bboxes, True, True, True, True, True, True, True, True, True)
    # img_pre.show()
